...
def buscar_query(query_json):
    ...
    try:
        ...
        resposta.raise_for_status()  # Verificador se a resposta da requests é 200-299 ou diferente, caso seja -> Exceção
        dados = resposta.json()
        return dados["hits"]["hits"]  # Acessando bloco com a informação
    except Exception as e:
        print(f"Erro no Backend: {e}")  # Log no terminal
        return []

def buscar_filmes(termo_digitado, categorias_selecionadas):
